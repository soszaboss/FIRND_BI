...
from .serializers import (
    ...
    DemandeSoumissionStatusSerializer,
    EtudiantSerializer,
    FileSerializer,
)
...
from .models import (
    Diploma,
    DemandeSoumission,
    DemandeSoumissionStatus,
    Etudiant,
    TypeDiplome,
)
from django.http import FileResponse, HttpResponse
import os
from .utils.download_and_watermark_document import (
    download_pdf_file,
...
@extend_schema(responses={200: FileSerializer})
@api_view(["GET"])
def file_watermarked(request, url):
    ...
    if filepath:
        ...
        return Response(
            {"error": "Échec du téléchargement et du filigrane du PDF."},
...
def signer_pdf(request, url):
    try:
        ...
        response = FileResponse(pdf_file, content_type="application/pdf")
        response["Content-Disposition"] = (
            f'attachment; filename="{os.path.basename(document)}"'
        )
...
@extend_schema(request=FileSerializer, responses={200: FileSerializer})
@api_view(["POST"])
def verifier_pdf(request):
    try:
        ...
        if not fichier_pdf:
            return Response(
                {"message": "Veuillez insérer un document."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if est_valide:
            return Response(
                {"message": "La signature est valide."}, status=status.HTTP_200_OK
            )
        else:
            return Response(
...
@extend_schema(responses={200: DemandeSoumissionStatusSerializer(many=True)})
@api_view(["GET"])
...
@extend_schema_view(
    post=extend_schema(request=FileSerializer, responses={201: FileSerializer}),
    get=extend_schema(responses={200: FileSerializer(many=True)}),
)
class SauvegardeJSON(APIView):
    def post(self, request):
        ...
        try:
            ...
            if os.path.exists(os.getcwd() + "/media/soumission.json"):
                ...
                existing_data = (
                    []
                )  # Créer une nouvelle liste si le fichier n'existe pas
            ...
            return Response(
                "Données sauvegardées avec succès !", status=status.HTTP_201_CREATED
            )
        except Exception as e:
            return Response(
                f"Erreur lors de la sauvegarde des données : {str(e)}",
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def get(self, request):
        try:
            ...
            return Response(
                f"Erreur lors de la lecture des données : {str(e)}",
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            response = requests.post(
                url + "templates/pdf", json=request.data, headers=headers
            )
            data = response.json()
            tem_id = data.get("id")
...
class TelechargerClePubliqueView(GenericAPIView):
    ...
    )  # Utilisez un sérialiseur vide si vous n'avez pas besoin de validation
    ...
    def get(self, request):
        ...
        if os.path.exists(file_path):
            return FileResponse(open(file_path, "rb"), as_attachment=True)
        return Response(
            {"erreur": "Fichier non trouvé"}, status=status.HTTP_404_NOT_FOUND
        )
